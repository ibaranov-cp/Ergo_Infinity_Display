...
def send_char(array,c,x,y): #Enter a char at location x,y using small 8x8 font
	for w in range (8):	
		array[x+w][y:y+8] = list(format(Small_Fonts8x8[(ord(c)-32)*8+w], '08b'))
		
def send_string(array,str,x,y):
	for i, c in enumerate (str):
		send_char(array,c,x+i*8,y)

for x in range (54,74):
	for y in range (6,26):
		lcd[x][y] = 1
		
def clear(array): # Clean lcd array and clean screen
...
def send(array): #Pass an array, for updating the whole screen, slow!
...
def update_pixel(array,x,y,val): #Update a single pixel at location x,y, with either 1 or 0
	array[x][y]= val;
	ypose = int(floor(y/8))
	val = ""
	for w in range (7,-1,-1):
...
if __name__ == '__main__':		
	...
	print "\n\rTiming Tests"
	print "Update Full LCD: " + str(timeit("send(lcd)",setup="from __main__ import send,lcd",number=1)) # Send 3 dot test pattern
	print "Clear LCD: " + str(timeit("clear(lcd)",setup="from __main__ import clear,lcd",number=1)) # Clear LCD array, and clear screen
	print "Update 1 individual pixel: " + str(timeit("update_pixel(lcd,1,1,1)",setup="from __main__ import update_pixel,lcd",number=1)) # Send individual pixels, while keeping exisiting data from array
